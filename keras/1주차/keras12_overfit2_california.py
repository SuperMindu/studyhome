...
#2.  모델 구성
model = Sequential()
model.add(Dense(124, input_dim=8))
model.add(Dense(124))
model.add(Dense(124))
model.add(Dense(124))
model.add(Dense(1))
...
hist = model.fit(x_train, y_train, epochs=500, batch_size=100, verbose=1, validation_split=0.2) #verbose = 매개변수 (중간 훈련과정 스킵, 시간 단축 가능)
