...
# 훈련량 (epochs)를 훨씬 줄여도 loss값을 구할 수 있음

#3. 컴파일, 훈련
# 컴터가 알아듣게 훈련시킴. 그게 컴파일. 
# y = wx+b 최적의 weight 값을 구하기 위한 최소의 mse값을 찾음
model.compile(loss='mse', optimizer='adam') 
# 평균 제곱 에러 m s e <- 이 값은 작을수록 좋음. 
# optimizer='adam'은 mse값을 감축시키는 역할. adam이 85퍼 이상이므로 꽤 괜
model.fit(x, y, epochs=200, batch_size=1) 
# epoch 훈련량을 의미.
# batch_size 몇개씩 데이터를 넣을지 지정해줌. 이게 작을수록 값이 정교해짐 시간 성능 속도?..
...
result = model.predict([6]) # 새로운 x값 [6]을 predict(예측)할 결과
print('6의 예측값 : ', result)
