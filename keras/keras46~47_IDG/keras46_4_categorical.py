...
# model = Sequential()
# model.add(Conv2D(20,(2,2), input_shape=(150,150,1), activation='relu'))
...
hist = model.fit_generator(xy_train, epochs=100, steps_per_epoch=32, # <- 데이터 셋을 배치사이즈로 나눈 덩어리값. 보통 이 값을 넣어줌. 여기엔 따로 배치 사이즈가 없음. 전체데이터/batch = 160/5 = 32
                    validation_data=xy_test, # <- 근데 여기서는 발리데이션을 test 데이터로 
                    validation_steps=24 # <- 알아서 찾아보기 
                    )
